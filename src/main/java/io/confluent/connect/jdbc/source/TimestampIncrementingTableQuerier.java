...
import io.confluent.connect.jdbc.source.dialect.TimestampStatementAdapter;
...
public class TimestampIncrementingTableQuerier extends TableQuerier {
  ...
  private DbDialectTimestampIncrementingOffset offset;
  ...
  public TimestampIncrementingTableQuerier(QueryMode mode, String name, String topicPrefix,
    ...
    this.offset = GenericTimestampIncrementingOffset.fromMap(offsetMap);
  }

  public TimestampIncrementingTableQuerier(QueryMode mode, String name, String topicPrefix,
                                           String timestampColumn, String incrementingColumn,
                                           Map<String, Object> offsetMap, Long timestampDelay,
                                           String schemaPattern, boolean mapNumerics, JdbcSourceTaskConfig config) {
    super(mode, name, topicPrefix, schemaPattern, mapNumerics);
    this.timestampColumn = timestampColumn;
    this.incrementingColumn = incrementingColumn;
    this.timestampDelay = timestampDelay;
    this.config = config;
    this.offset = DbDialectTimestampIncrementingOffset.fromConnectionStringAndMap(
        config.getString(JdbcSourceConnectorConfig.CONNECTION_URL_CONFIG),
        offsetMap,
        config.getBoolean(JdbcSourceConnectorConfig.SQL_SERVER_ROWVERSION_TIMESTAMP_CONFIG)
    );
  }
  ...
  protected ResultSet executeQuery() throws SQLException {
    if (incrementingColumn != null && timestampColumn != null) {
      ...
      stmt = TimestampStatementAdapter.getTimeStampPreparedStatement(offset, timestampDelay, stmt);
    }
  ...
  // Visible for testing
  DbDialectTimestampIncrementingOffset extractOffset(Schema schema, Struct record) {
    final Object extractedTimestamp;
    if (timestampColumn != null) {
      extractedTimestamp = TimestampStatementAdapter.extractTimeStampRecordAssertion(offset, record, timestampColumn);
    } else {
    ...
    return DbDialectTimestampIncrementingOffset.fromConnectionString(
        config.getString(JdbcSourceConnectorConfig.CONNECTION_URL_CONFIG),
        extractedTimestamp,
        extractedId,
        config.getBoolean(JdbcSourceConnectorConfig.SQL_SERVER_ROWVERSION_TIMESTAMP_CONFIG)
